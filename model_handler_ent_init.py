import SVM_SGD_ENT_POS_init as svm_sgd_ent
import evaluator as e
import operator
import sys

class svm_ent_models_handler_pos_init():
    def __init__(self, C_array,Gamma_array):
        self.models = []
        for C in C_array:
            for Gamma in Gamma_array:
                self.models.append(svm_sgd_ent.svm_sgd_entropy_pos_init(C,Gamma))
    ...
    def fit_model_on_train_set_and_choose_best_opt(self, X, X_i, y_i, validation_indices, fold, queries,score_opt,gamma,init_models,evaluator):
        print("fitting models on fold", fold)
        weights = {}
        scores = {}
        for svm in self.models:
            sys.stdout.flush()
            svm.fit(X_i, y_i,init_models.weights_index[fold])
            weights[(svm.C,svm.Gamma)] = svm.w
