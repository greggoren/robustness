...
class svm_sgd_doubly(svm_s.svm_sgd):
    def __init__(self, Lambda1=None,Lambda2=None):
    ...
    def fit(self,X,y):
        ...
        for t in range(iterations):#itarating over examples
            ...
            if not self.check_prediction(y_k):
                self.w = self.w-lr*Lambda1*self.L1_norm_subgradient(number_of_features) + lr*y_k - lr*self.Lambda2*self.w
            else:
    ...
    def predict_opt(self,X,queries,test_indices,eval,score,validation=None):
        ...
        for index in test_indices:
            results[index] = np.dot(self.w,X[index].T)
        return eval.create_trec_eval_file_opt(test_indices,queries,results,str(self.Lambda1)+"_"+str(self.Lambda2),score,self.Lambda1,self.Lambda2,validation)
