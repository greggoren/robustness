...
class eval:
    ...
    def create_trec_eval_file(self, test_indices, queries, results, model,
        ...
        for index in test_indices:
            trec_file_access.write(
                self.set_qid_for_trec(queries[index]) + " Q0 " + self.doc_name_index[index] + " " + str(0) + " " + str(
                    results[index]) + " seo\n")
    ...
    def run_trec_eval_on_test_correlation(self, trec_file, trees, leaves):
        score_data = []
        print("last stats:")
        for metric in self.metrics:
            command = "./trec_eval -m " + metric + " " + params.qrels + " " + trec_file
            for output_line in self.run_command(command):
                ...
                score = output_line.split()[-1].rstrip()
                score_data.append((metric, str(score)))
        summary_file = open("leaves_relevance_for_correlation_LambdaMart", 'a')
    ...
    def create_qrels_file(self, X, y, queries):
        ...
        for i in range(len(X)):
            qrels.write(
                self.set_qid_for_trec(queries[i]) + " 0 " + self.doc_name_index[i] + " " + str(int(y[i])) + "\n")
        qrels.close()
