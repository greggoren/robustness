...
class analyze:
    ...
    def create_table(self, competition_data, svms, banned_queries):
        ...
        for key in keys:
            ...
            change_max = str(round(np.mean(change_rate[key][0]), 3))
            wc_max_for_pearson.append(change_max)
            change_mean = str(round(np.mean(change_rate[key][2]), 3))
            ...
            change_weighted = str(round(np.mean(change_rate[key][1]), 3))
